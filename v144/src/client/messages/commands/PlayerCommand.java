...
public class PlayerCommand {
        ...
        public int execute(MapleClient c, String[] splitted) {
                ...
                if (splitted.length == 2) {
                    ...
                    if (!ii.itemExists(itemid)) {
                        c.getPlayer().dropMessage(6, "This item doesn't exist.");
                        return 0;
                    }
                         FileoutputUtil.log("MSI_Log.txt", "[MSI VerificationWait]" + c.getPlayer().getName() + " has made a MSI with the following info: ItemID " + itemid + " MSIPOINTS NOW: " + c.getPlayer().getMSIPoints());
                    if (type.equals(MapleInventoryType.EQUIP) && !c.getPlayer().getInventory(MapleInventoryType.EQUIP).isFull()) {
                        ...
                        c.getPlayer().saveToDB(false, false);
                                                FileoutputUtil.log("MSI_Log.txt", "[MSI Confirmed]" + c.getPlayer().getName() + " has made a MSI with the following info: ItemID " + itemid + " MSIPOINTS NOW: " + c.getPlayer().getMSIPoints());
                    } else {
    ...
    public static class wtfcommand extends CommandExecute {

        public int execute(MapleClient c, String[] splitted) {
  //           c.getPlayer().getClient().getSession().write(CWvsContext.updateSP(c.getPlayer()));
