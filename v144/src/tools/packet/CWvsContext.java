...
public class CWvsContext {
    ...
    public static byte[] updatePlayerStats(final Map<MapleStat, Long> mystats, final MapleCharacter chr) {
        return updatePlayerStats(mystats, false, chr);
  ...
  public static byte[] updateSpecialStat(String stat, int array, int mode, int amount) {
    ...
    public static byte[] charInfo(MapleCharacter chr, boolean isSelf) {
        ...
        if (chr.getMarriageId() > 0) {
        	mplew.write(HexTool.hex("85 50 00 00 C6 C3 3D 00 A7 BE 3D 00 03 00 F4 F8 10 00 F4 F8 10 00")); // you ar married now bla
        	mplew.writeAsciiString(chr.getClient().getNameById(chr.getMarriageId()), 13);
    ...
    public static class BuffPacket {
        ...
        public static byte[] removeDamageCap(MapleCharacter chr, Map<MapleBuffStat, Integer> statups) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            ...
            mplew.writeInt(10);
            mplew.writeInt(0);
            mplew.writeInt(2100000000); // duration
            mplew.writeInt(1); // important
            mplew.writeInt(0);
            mplew.writeInt(2050000000); // dmg
            mplew.writeInt(0);
            mplew.writeInt(2100000000); // duration
            mplew.writeLong(0);
            mplew.write(0);
...
///*      */     }
            ...
            for (Map.Entry stat : statups.entrySet()) {
                ...
                if (((MapleBuffStat) stat.getKey()).canStack()) {
                    ...
                    if (buffid == 61111004) {
                      ...
                      //  mplew.writeLong(((Integer) stat.getValue()).longValue()); //could be int in v143
                        mplew.writeInt((Integer) stat.getValue());
                        mplew.write(HexTool.hex("FF FF FF 7F 00 00 00 00")); // Hawt 143 has this (idk why)
                    }
                } else {
    ...
    public static class InfoPacket {
        ...
        public static byte[] GainEXP_Monster(long gain, boolean white, int partyinc, int Class_Bonus_EXP, int Equipment_Bonus_EXP, int Premium_Bonus_EXP) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//            for (MaplePartyCharacter partychar : partymembers) {
////Â¾Ã†Ã�Ã· Â¹ÂºÃ�Ã¶Â´Ã‚ Â¸Ã°Â¸Â£Â°ÃšÃ€Â½
