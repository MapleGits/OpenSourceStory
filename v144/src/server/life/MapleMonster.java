...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public final int getLevel() {
        ...
        int PDRate = stats.getPDRate();
        if (ostats != null) {
            PDRate = ostats.PDRate;
        }
        MonsterStatusEffect pdr = getBuff(MonsterStatus.WDEF);
        if (pdr != null) {
            PDRate += pdr.getX();
        }
        return PDRate;
    ...
    public final int getMDRate() {
        int MDRate = stats.getMDRate();
        if (ostats != null) {
            MDRate = ostats.MDRate;
        }
        MonsterStatusEffect pdr = getBuff(MonsterStatus.MDEF);
        if (pdr != null) {
            MDRate += pdr.getX();
        }
        return MDRate;
  ...
  /*  public final void applyStatus(final MapleCharacter from, MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        System.out.println("start applystatus");
        ...
        if (isBuffed(MonsterStatus.HYPNOTIZE) || MapleLifeFactory.isDmgSponge(getId()) || stats.isInvincible()) {
            return;
        }
        ...
        if (stats.isBoss()) {
            ...
            if (checkboss && stat != (MonsterStatus.IMPRINT) && stat != (MonsterStatus.SPEED) && stat != (MonsterStatus.NINJA_AMBUSH) && stat != (MonsterStatus.WATK) && stat != (MonsterStatus.WDEF) && stat != (MonsterStatus.MATK) && stat != (MonsterStatus.MDEF) && stat != (MonsterStatus.POISON) && stat != MonsterStatus.BURN && stat != (MonsterStatus.DARKNESS) && stat != (MonsterStatus.MAGIC_CRASH)) {
                return;
            }
            if (stat == MonsterStatus.IMPRINT) {
                status.setValue(status.getStati(), status.getX() * 2);
            }
            //don't magic crash cygnus boss
            if ((getId() == 8850011 || (getId() >= 9400589 && getId() <= 9400593)) && stat == MonsterStatus.MAGIC_CRASH) {
                return;
            }
        }
        if (isBuffed(MonsterStatus.DOT_IMMUNITY) && (stat == MonsterStatus.POISON || stat == MonsterStatus.BURN)) {
            return;
        } 
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.BURN) {
			...
			if (countPoisons > eff.getDOTStack()) {
				return;
			}
        }
        if (poison && getHp() > 1 && eff != null && eff.getDOTTime() > 0) {
            duration = (eff.getDOTTime() * 1000) + (from.getStat().dotTime * 1000);
        }
        ...
        if (poison && getHp() > 1) {
            double maxDamage = ((eff.getDOT() + from.getStat().getDamageIncrease(eff.getSourceId())) * from.getDamageIncrease(getObjectId()) * (from.getStat().getCurrentMaxBaseDamage() / 100.0) * ((stats.isBoss() ? from.getStat().bossdam_r : from.getStat().dam_r) / 100.0));
            maxDamage = (maxDamage / 100) * (DamageParse.ElementalStaffAttackBonus(skilz == null || from.getBuffedValue(MapleBuffStat.ELEMENT_RESET) != null ? Element.NEUTRAL : skilz.getElement(), stats.getEffectiveness(skilz == null || from.getBuffedValue(MapleBuffStat.ELEMENT_RESET) != null ? Element.NEUTRAL : skilz.getElement()).getValue(), from.getStat()));
            maxDamage -= (maxDamage * (Math.max(Math.min(99, (skilz == null || !skilz.isMagic() ? getPDRate() : getMDRate()) - (((skilz == null || !skilz.isMagic() ? getPDRate() : getMDRate()) * (from.getStat().ignoreTargetDEF + eff.getIgnoreMob())) / 100)), 0) / 100.0));
            maxDamage -= (maxDamage * (Math.max(Math.min(99, getLevel() - from.getLevel()), 0) / 100.0));

            status.setValue(status.getStati(), Integer.valueOf((int) Math.max(1, Math.min(Integer.MAX_VALUE, maxDamage))));
            status.setPoisonSchedule((int) ((duration / 1000) * status.getX()), from);
            ...
            status.setPoisonSchedule(Integer.valueOf((int) (eff.getDamage() * (from.getStat().getCurrentMaxBaseDamage() / 100.0) * from.getDamageIncrease(getObjectId()) * ((stats.isBoss() ? from.getStat().bossdam_r : from.getStat().dam_r) / 100.0))), from);

        } else if (statusSkill == 4121004 || statusSkill == 4221004) {
            status.setValue(status.getStati(), Math.min(Short.MAX_VALUE, Integer.valueOf((int) (eff.getDamage() * from.getDamageIncrease(getObjectId()) * (from.getStat().getCurrentMaxBaseDamage() / 100.0) * ((stats.isBoss() ? from.getStat().bossdam_r : from.getStat().dam_r) / 100.0)))));
...
*/
       public final void applyStatus(MapleCharacter from, MonsterStatusEffect status, boolean poison, long duration, boolean checkboss, MapleStatEffect eff) {
        ...
        if (((this.stats.isFriendly()) || (isFake())) && ((stat == MonsterStatus.STUN) || (stat == MonsterStatus.SPEED) || (stat == MonsterStatus.POISON) || (stat == MonsterStatus.VENOMOUS_WEAPON))) {
            return;
        }
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr, long damage) {
		...
		if (damage > status.getPoisonSchedule()) {
			damage = status.getPoisonSchedule();
		}
		...
		status.setPoisonSchedule(status.getPoisonSchedule() - (int) damage, chr);
    }
